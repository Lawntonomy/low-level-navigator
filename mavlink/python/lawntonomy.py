...
Generated from: ('lawntonomy.xml,minimal.xml',)
...
# LAWN_NAV_STATE
enums["LAWN_NAV_STATE"] = Enum()
enums["LAWN_NAV_STATE"].bitmask = False
LAWN_NAV_PRECAL_IDLE = 0
enums["LAWN_NAV_STATE"][0] = EnumEntry("LAWN_NAV_PRECAL_IDLE", """No valid calibration. Zero speed. Drive not enabled.""")
LAWN_NAV_CALIBRATING = 1
enums["LAWN_NAV_STATE"][1] = EnumEntry("LAWN_NAV_CALIBRATING", """Calibration in progress. Motion may be commanded internally.""")
LAWN_NAV_IDLE = 2
enums["LAWN_NAV_STATE"][2] = EnumEntry("LAWN_NAV_IDLE", """Valid calibration, armed or not, no guidance commands being executed.""")
LAWN_NAV_ACTIVE = 3
enums["LAWN_NAV_STATE"][3] = EnumEntry("LAWN_NAV_ACTIVE", """Executing guidance commands.""")
LAWN_NAV_EXITING = 4
enums["LAWN_NAV_STATE"][4] = EnumEntry("LAWN_NAV_EXITING", """No fresh commands; ramping both wheels to zero. SAF-1.""")
LAWN_NAV_FAULT = 5
enums["LAWN_NAV_STATE"][5] = EnumEntry("LAWN_NAV_FAULT", """Latched fault. Drive deasserted. Requires explicit clear.""")
LAWN_NAV_STATE_ENUM_END = 6
enums["LAWN_NAV_STATE"][6] = EnumEntry("LAWN_NAV_STATE_ENUM_END", """""")

# LAWN_WHEEL_FLAGS
enums["LAWN_WHEEL_FLAGS"] = Enum()
enums["LAWN_WHEEL_FLAGS"].bitmask = True
LAWN_WHEEL_LEFT_VALID = 1
enums["LAWN_WHEEL_FLAGS"][1] = EnumEntry("LAWN_WHEEL_LEFT_VALID", """Left wheel speed is fresh. SAF-20.""")
LAWN_WHEEL_RIGHT_VALID = 2
enums["LAWN_WHEEL_FLAGS"][2] = EnumEntry("LAWN_WHEEL_RIGHT_VALID", """Right wheel speed is fresh. SAF-20.""")
LAWN_WHEEL_LEFT_STALL = 4
enums["LAWN_WHEEL_FLAGS"][4] = EnumEntry("LAWN_WHEEL_LEFT_STALL", """Left wheel stall detected. SAF-21.""")
LAWN_WHEEL_RIGHT_STALL = 8
enums["LAWN_WHEEL_FLAGS"][8] = EnumEntry("LAWN_WHEEL_RIGHT_STALL", """Right wheel stall detected. SAF-21.""")
LAWN_WHEEL_DIR_UNMEASURED = 16
enums["LAWN_WHEEL_FLAGS"][16] = EnumEntry(
    "LAWN_WHEEL_DIR_UNMEASURED",
    """Direction is inferred from command, not measured. Always set until
        direction-observable encoders exist. See ADR-0002.""",
)
LAWN_WHEEL_FLAGS_ENUM_END = 17
enums["LAWN_WHEEL_FLAGS"][17] = EnumEntry("LAWN_WHEEL_FLAGS_ENUM_END", """""")

# LAWN_FAULT_CODE
enums["LAWN_FAULT_CODE"] = Enum()
enums["LAWN_FAULT_CODE"].bitmask = False
LAWN_FAULT_NONE = 0
enums["LAWN_FAULT_CODE"][0] = EnumEntry("LAWN_FAULT_NONE", """""")
LAWN_FAULT_CMD_TIMEOUT = 1
enums["LAWN_FAULT_CODE"][1] = EnumEntry("LAWN_FAULT_CMD_TIMEOUT", """No valid command within the timeout. SAF-1.""")
LAWN_FAULT_LINK_DEGRADED = 2
enums["LAWN_FAULT_CODE"][2] = EnumEntry("LAWN_FAULT_LINK_DEGRADED", """Link quality below threshold while still nominally up. SAF-54.""")
LAWN_FAULT_WHEEL_STALL = 3
enums["LAWN_FAULT_CODE"][3] = EnumEntry("LAWN_FAULT_WHEEL_STALL", """""")
LAWN_FAULT_WHEEL_INVALID = 4
enums["LAWN_FAULT_CODE"][4] = EnumEntry("LAWN_FAULT_WHEEL_INVALID", """Wheel speed stale or implausible. SAF-20.""")
LAWN_FAULT_DIR_MISMATCH = 5
enums["LAWN_FAULT_CODE"][5] = EnumEntry("LAWN_FAULT_DIR_MISMATCH", """Commanded direction disagrees with chassis motion. SAF-23.""")
LAWN_FAULT_TILT = 6
enums["LAWN_FAULT_CODE"][6] = EnumEntry("LAWN_FAULT_TILT", """""")
...
MAVLINK_MSG_ID_LAWN_FAULT_EVENT = 42013
MAVLINK_MSG_ID_LAWN_TIMESYNC = 42020
MAVLINK_MSG_ID_LAWN_IMU_RAW = 42030
MAVLINK_MSG_ID_HEARTBEAT = 0


class MAVLink_lawn_drive_cmd_message(MAVLink_message):
    """
    Wheel speed request, high-level to low-level. A request, not a
    command:       the low-level tier decides whether to honour it
    (ADR-0001). Rejected unless fresh       (SAF-52) and the machine
    is armed (SAF-11).
    """

    id = MAVLINK_MSG_ID_LAWN_DRIVE_CMD
    msgname = "LAWN_DRIVE_CMD"
    fieldnames = ["left_drpm", "right_drpm"]
    ordered_fieldnames = ["left_drpm", "right_drpm"]
    fieldtypes = ["int16_t", "int16_t"]
    fielddisplays_by_name: Dict[str, str] = {}
    fieldenums_by_name: Dict[str, str] = {}
    fieldunits_by_name: Dict[str, str] = {"left_drpm": "drpm", "right_drpm": "drpm"}
    ...
    def __init__(self, left_drpm: int, right_drpm: int):
        MAVLink_message.__init__(self, MAVLink_lawn_drive_cmd_message.id, MAVLink_lawn_drive_cmd_message.msgname)
        self._fieldnames = MAVLink_lawn_drive_cmd_message.fieldnames
        self._instance_field = MAVLink_lawn_drive_cmd_message.instance_field
        self._instance_offset = MAVLink_lawn_drive_cmd_message.instance_offset
        self.left_drpm = left_drpm
        self.right_drpm = right_drpm

    def pack(self, mav: "MAVLink", force_mavlink1: bool = False) -> bytes:
        return self._pack(mav, self.crc_extra, self.unpacker.pack(self.left_drpm, self.right_drpm), force_mavlink1=force_mavlink1)


# Define name on the class for backwards compatibility (it is now msgname).
# Done with setattr to hide the class variable from mypy.
setattr(MAVLink_lawn_drive_cmd_message, "name", mavlink_msg_deprecated_name_property())


class MAVLink_lawn_arm_cmd_message(MAVLink_message):
    """
    Explicit arming. SAF-11 forbids driving on power-up without this.
    """

    id = MAVLINK_MSG_ID_LAWN_ARM_CMD
    msgname = "LAWN_ARM_CMD"
    fieldnames = ["magic", "arm"]
    ordered_fieldnames = ["magic", "arm"]
    ...
    def __init__(self, magic: int, arm: int):
        MAVLink_message.__init__(self, MAVLink_lawn_arm_cmd_message.id, MAVLink_lawn_arm_cmd_message.msgname)
        self._fieldnames = MAVLink_lawn_arm_cmd_message.fieldnames
        self._instance_field = MAVLink_lawn_arm_cmd_message.instance_field
        self._instance_offset = MAVLink_lawn_arm_cmd_message.instance_offset
        self.magic = magic
        self.arm = arm

    def pack(self, mav: "MAVLink", force_mavlink1: bool = False) -> bytes:
        return self._pack(mav, self.crc_extra, self.unpacker.pack(self.magic, self.arm), force_mavlink1=force_mavlink1)


# Define name on the class for backwards compatibility (it is now msgname).
# Done with setattr to hide the class variable from mypy.
setattr(MAVLink_lawn_arm_cmd_message, "name", mavlink_msg_deprecated_name_property())


class MAVLink_lawn_stop_req_message(MAVLink_message):
    """
    Request an immediate ramp to zero and disarm. NOT an emergency
    stop:       SAF-40 requires a stop independent of both processors,
    which this is not. Named to       prevent it being mistaken for
    one.
    """

    id = MAVLINK_MSG_ID_LAWN_STOP_REQ
    msgname = "LAWN_STOP_REQ"
    fieldnames = ["reason"]
    ordered_fieldnames = ["reason"]
    fieldtypes = ["uint8_t"]
    fielddisplays_by_name: Dict[str, str] = {}
    fieldenums_by_name: Dict[str, str] = {}
    fieldunits_by_name: Dict[str, str] = {}
    ...
    def __init__(self, reason: int):
        MAVLink_message.__init__(self, MAVLink_lawn_stop_req_message.id, MAVLink_lawn_stop_req_message.msgname)
        self._fieldnames = MAVLink_lawn_stop_req_message.fieldnames
        self._instance_field = MAVLink_lawn_stop_req_message.instance_field
        self._instance_offset = MAVLink_lawn_stop_req_message.instance_offset
        self.reason = reason

    def pack(self, mav: "MAVLink", force_mavlink1: bool = False) -> bytes:
        return self._pack(mav, self.crc_extra, self.unpacker.pack(self.reason), force_mavlink1=force_mavlink1)
...
class MAVLink_lawn_nav_status_message(MAVLink_message):
    """
    Navigator state. Sent at the telemetry rate.
    """

    id = MAVLINK_MSG_ID_LAWN_NAV_STATUS
    msgname = "LAWN_NAV_STATUS"
    fieldnames = ["t_meas_us", "cmd_age_ms", "nav_state", "armed", "fault"]
    ordered_fieldnames = ["t_meas_us", "cmd_age_ms", "nav_state", "armed", "fault"]
    fieldtypes = ["uint64_t", "uint16_t", "uint8_t", "uint8_t", "uint8_t"]
    fielddisplays_by_name: Dict[str, str] = {}
    fieldenums_by_name: Dict[str, str] = {"nav_state": "LAWN_NAV_STATE", "fault": "LAWN_FAULT_CODE"}
    fieldunits_by_name: Dict[str, str] = {"t_meas_us": "us", "cmd_age_ms": "ms"}
    native_format = bytearray(b"<QHBBB")
    orders = [0, 1, 2, 3, 4]
    lengths = [1, 1, 1, 1, 1]
    array_lengths = [0, 0, 0, 0, 0]
    crc_extra = 131
    unpacker = struct.Struct("<QHBBB")
    instance_field = None
    instance_offset = -1

    def __init__(self, t_meas_us: int, cmd_age_ms: int, nav_state: int, armed: int, fault: int):
        MAVLink_message.__init__(self, MAVLink_lawn_nav_status_message.id, MAVLink_lawn_nav_status_message.msgname)
        self._fieldnames = MAVLink_lawn_nav_status_message.fieldnames
        self._instance_field = MAVLink_lawn_nav_status_message.instance_field
        self._instance_offset = MAVLink_lawn_nav_status_message.instance_offset
        self.t_meas_us = t_meas_us
        self.cmd_age_ms = cmd_age_ms
        self.nav_state = nav_state
        self.armed = armed
        self.fault = fault

    def pack(self, mav: "MAVLink", force_mavlink1: bool = False) -> bytes:
        return self._pack(mav, self.crc_extra, self.unpacker.pack(self.t_meas_us, self.cmd_age_ms, self.nav_state, self.armed, self.fault), force_mavlink1=force_mavlink1)


# Define name on the class for backwards compatibility (it is now msgname).
# Done with setattr to hide the class variable from mypy.
setattr(MAVLink_lawn_nav_status_message, "name", mavlink_msg_deprecated_name_property())


class MAVLink_lawn_wheel_state_message(MAVLink_message):
    """
    Measured and commanded wheel speeds, timestamped at measurement.
    """

    id = MAVLINK_MSG_ID_LAWN_WHEEL_STATE
    msgname = "LAWN_WHEEL_STATE"
    fieldnames = ["t_meas_us", "left_drpm", "right_drpm", "left_cmd_drpm", "right_cmd_drpm", "flags"]
    ordered_fieldnames = ["t_meas_us", "left_drpm", "right_drpm", "left_cmd_drpm", "right_cmd_drpm", "flags"]
    fieldtypes = ["uint64_t", "int16_t", "int16_t", "int16_t", "int16_t", "uint8_t"]
    fielddisplays_by_name: Dict[str, str] = {"flags": "bitmask"}
    fieldenums_by_name: Dict[str, str] = {"flags": "LAWN_WHEEL_FLAGS"}
    fieldunits_by_name: Dict[str, str] = {"t_meas_us": "us", "left_drpm": "drpm", "right_drpm": "drpm", "left_cmd_drpm": "drpm", "right_cmd_drpm": "drpm"}
    native_format = bytearray(b"<QhhhhB")
    orders = [0, 1, 2, 3, 4, 5]
    lengths = [1, 1, 1, 1, 1, 1]
    array_lengths = [0, 0, 0, 0, 0, 0]
    crc_extra = 90
    unpacker = struct.Struct("<QhhhhB")
    instance_field = None
    instance_offset = -1

    def __init__(self, t_meas_us: int, left_drpm: int, right_drpm: int, left_cmd_drpm: int, right_cmd_drpm: int, flags: int):
        MAVLink_message.__init__(self, MAVLink_lawn_wheel_state_message.id, MAVLink_lawn_wheel_state_message.msgname)
        self._fieldnames = MAVLink_lawn_wheel_state_message.fieldnames
        self._instance_field = MAVLink_lawn_wheel_state_message.instance_field
        self._instance_offset = MAVLink_lawn_wheel_state_message.instance_offset
        self.t_meas_us = t_meas_us
        self.left_drpm = left_drpm
        self.right_drpm = right_drpm
        self.left_cmd_drpm = left_cmd_drpm
        self.right_cmd_drpm = right_cmd_drpm
        self.flags = flags

    def pack(self, mav: "MAVLink", force_mavlink1: bool = False) -> bytes:
        return self._pack(mav, self.crc_extra, self.unpacker.pack(self.t_meas_us, self.left_drpm, self.right_drpm, self.left_cmd_drpm, self.right_cmd_drpm, self.flags), force_mavlink1=force_mavlink1)


# Define name on the class for backwards compatibility (it is now msgname).
# Done with setattr to hide the class variable from mypy.
setattr(MAVLink_lawn_wheel_state_message, "name", mavlink_msg_deprecated_name_property())


class MAVLink_lawn_link_stats_message(MAVLink_message):
    """
    Windowed link quality. SAF-54 and SAF-55: a degrading link must be
    observable before it becomes a stop.
    """

    id = MAVLINK_MSG_ID_LAWN_LINK_STATS
    msgname = "LAWN_LINK_STATS"
    fieldnames = ["t_meas_us", "frames_accepted", "frames_rejected", "dropped_tx", "window_ms", "heartbeats_missed", "quality"]
    ordered_fieldnames = ["t_meas_us", "frames_accepted", "frames_rejected", "dropped_tx", "window_ms", "heartbeats_missed", "quality"]
    fieldtypes = ["uint64_t", "uint32_t", "uint32_t", "uint32_t", "uint16_t", "uint16_t", "uint8_t"]
    fielddisplays_by_name: Dict[str, str] = {}
    fieldenums_by_name: Dict[str, str] = {}
    fieldunits_by_name: Dict[str, str] = {"t_meas_us": "us", "window_ms": "ms"}
    native_format = bytearray(b"<QIIIHHB")
    orders = [0, 1, 2, 3, 4, 5, 6]
    lengths = [1, 1, 1, 1, 1, 1, 1]
    array_lengths = [0, 0, 0, 0, 0, 0, 0]
    crc_extra = 214
    unpacker = struct.Struct("<QIIIHHB")
    instance_field = None
    instance_offset = -1

    def __init__(self, t_meas_us: int, frames_accepted: int, frames_rejected: int, dropped_tx: int, window_ms: int, heartbeats_missed: int, quality: int):
        MAVLink_message.__init__(self, MAVLink_lawn_link_stats_message.id, MAVLink_lawn_link_stats_message.msgname)
        self._fieldnames = MAVLink_lawn_link_stats_message.fieldnames
        self._instance_field = MAVLink_lawn_link_stats_message.instance_field
        self._instance_offset = MAVLink_lawn_link_stats_message.instance_offset
        self.t_meas_us = t_meas_us
        self.frames_accepted = frames_accepted
        self.frames_rejected = frames_rejected
        self.dropped_tx = dropped_tx
        self.window_ms = window_ms
        self.heartbeats_missed = heartbeats_missed
        self.quality = quality

    def pack(self, mav: "MAVLink", force_mavlink1: bool = False) -> bytes:
        return self._pack(mav, self.crc_extra, self.unpacker.pack(self.t_meas_us, self.frames_accepted, self.frames_rejected, self.dropped_tx, self.window_ms, self.heartbeats_missed, self.quality), force_mavlink1=force_mavlink1)


# Define name on the class for backwards compatibility (it is now msgname).
# Done with setattr to hide the class variable from mypy.
setattr(MAVLink_lawn_link_stats_message, "name", mavlink_msg_deprecated_name_property())


class MAVLink_lawn_fault_event_message(MAVLink_message):
    """
    Fault transition. Sent on change, not periodically.
    """

    id = MAVLINK_MSG_ID_LAWN_FAULT_EVENT
    msgname = "LAWN_FAULT_EVENT"
    fieldnames = ["t_meas_us", "code", "nav_state", "latched"]
    ordered_fieldnames = ["t_meas_us", "code", "nav_state", "latched"]
    fieldtypes = ["uint64_t", "uint16_t", "uint8_t", "uint8_t"]
    fielddisplays_by_name: Dict[str, str] = {}
    fieldenums_by_name: Dict[str, str] = {"code": "LAWN_FAULT_CODE", "nav_state": "LAWN_NAV_STATE"}
    fieldunits_by_name: Dict[str, str] = {"t_meas_us": "us"}
    native_format = bytearray(b"<QHBB")
    orders = [0, 1, 2, 3]
    lengths = [1, 1, 1, 1]
    array_lengths = [0, 0, 0, 0]
    crc_extra = 223
    unpacker = struct.Struct("<QHBB")
    instance_field = None
    instance_offset = -1

    def __init__(self, t_meas_us: int, code: int, nav_state: int, latched: int):
        MAVLink_message.__init__(self, MAVLink_lawn_fault_event_message.id, MAVLink_lawn_fault_event_message.msgname)
        self._fieldnames = MAVLink_lawn_fault_event_message.fieldnames
        self._instance_field = MAVLink_lawn_fault_event_message.instance_field
        self._instance_offset = MAVLink_lawn_fault_event_message.instance_offset
        self.t_meas_us = t_meas_us
        self.code = code
        self.nav_state = nav_state
    ...
    def pack(self, mav: "MAVLink", force_mavlink1: bool = False) -> bytes:
        return self._pack(mav, self.crc_extra, self.unpacker.pack(self.t_meas_us, self.code, self.nav_state, self.latched), force_mavlink1=force_mavlink1)


# Define name on the class for backwards compatibility (it is now msgname).
# Done with setattr to hide the class variable from mypy.
setattr(MAVLink_lawn_fault_event_message, "name", mavlink_msg_deprecated_name_property())


class MAVLink_lawn_timesync_message(MAVLink_message):
    """
    Four-timestamp round trip. Replaces standard TIMESYNC (111), which
    carries       only two timestamps and forces a symmetric-delay
    assumption. ADR-0007 fits offset AND       skew, so all four are
    carried.        Request (Pi): exchange_seq set, t1_us = Pi
    transmit time, t2_us = t3_us = 0.       Response (Pico):
    exchange_seq echoed, t1_us echoed unchanged, t2_us = receive time,
    t3_us = transmit time.        t2_us is captured at the start-bit
    edge of the request's first byte; t3_us is captured       as the
    response's last byte leaves the shift register. t3_us is a
    DELIBERATE exception       to ADR-0007 item 1: for a sync frame,
    transmit time IS the measurement.
    """

    id = MAVLINK_MSG_ID_LAWN_TIMESYNC
    msgname = "LAWN_TIMESYNC"
    fieldnames = ["t1_us", "t2_us", "t3_us", "exchange_seq"]
    ordered_fieldnames = ["t1_us", "t2_us", "t3_us", "exchange_seq"]
    fieldtypes = ["uint64_t", "uint64_t", "uint64_t", "uint8_t"]
    fielddisplays_by_name: Dict[str, str] = {}
    fieldenums_by_name: Dict[str, str] = {}
    fieldunits_by_name: Dict[str, str] = {"t1_us": "us", "t2_us": "us", "t3_us": "us"}
    native_format = bytearray(b"<QQQB")
    orders = [0, 1, 2, 3]
    lengths = [1, 1, 1, 1]
    array_lengths = [0, 0, 0, 0]
    crc_extra = 72
    unpacker = struct.Struct("<QQQB")
    instance_field = None
    instance_offset = -1

    def __init__(self, t1_us: int, t2_us: int, t3_us: int, exchange_seq: int):
        MAVLink_message.__init__(self, MAVLink_lawn_timesync_message.id, MAVLink_lawn_timesync_message.msgname)
        self._fieldnames = MAVLink_lawn_timesync_message.fieldnames
        self._instance_field = MAVLink_lawn_timesync_message.instance_field
        self._instance_offset = MAVLink_lawn_timesync_message.instance_offset
        self.t1_us = t1_us
        self.t2_us = t2_us
        self.t3_us = t3_us
    ...
    def pack(self, mav: "MAVLink", force_mavlink1: bool = False) -> bytes:
        return self._pack(mav, self.crc_extra, self.unpacker.pack(self.t1_us, self.t2_us, self.t3_us, self.exchange_seq), force_mavlink1=force_mavlink1)


# Define name on the class for backwards compatibility (it is now msgname).
# Done with setattr to hide the class variable from mypy.
setattr(MAVLink_lawn_timesync_message, "name", mavlink_msg_deprecated_name_property())


class MAVLink_lawn_imu_raw_message(MAVLink_message):
    """
    Raw inertial sample, timestamped at the sensor data-ready edge.
    Carried on       the BULK channel (USB CDC), not the command link.
    ADR-0006 keeps raw acquisition low and       estimation high.
    """

    id = MAVLINK_MSG_ID_LAWN_IMU_RAW
    msgname = "LAWN_IMU_RAW"
    fieldnames = ["t_meas_us", "ax", "ay", "az", "gx", "gy", "gz", "gap"]
    ordered_fieldnames = ["t_meas_us", "ax", "ay", "az", "gx", "gy", "gz", "gap"]
    fieldtypes = ["uint64_t", "int16_t", "int16_t", "int16_t", "int16_t", "int16_t", "int16_t", "uint8_t"]
    fielddisplays_by_name: Dict[str, str] = {}
    fieldenums_by_name: Dict[str, str] = {}
    fieldunits_by_name: Dict[str, str] = {"t_meas_us": "us"}
    native_format = bytearray(b"<QhhhhhhB")
    orders = [0, 1, 2, 3, 4, 5, 6, 7]
    lengths = [1, 1, 1, 1, 1, 1, 1, 1]
    array_lengths = [0, 0, 0, 0, 0, 0, 0, 0]
    crc_extra = 63
    unpacker = struct.Struct("<QhhhhhhB")
    instance_field = None
    instance_offset = -1

    def __init__(self, t_meas_us: int, ax: int, ay: int, az: int, gx: int, gy: int, gz: int, gap: int):
        MAVLink_message.__init__(self, MAVLink_lawn_imu_raw_message.id, MAVLink_lawn_imu_raw_message.msgname)
        self._fieldnames = MAVLink_lawn_imu_raw_message.fieldnames
        self._instance_field = MAVLink_lawn_imu_raw_message.instance_field
        self._instance_offset = MAVLink_lawn_imu_raw_message.instance_offset
        self.t_meas_us = t_meas_us
        self.ax = ax
        self.ay = ay
        self.az = az
    ...
    def pack(self, mav: "MAVLink", force_mavlink1: bool = False) -> bytes:
        return self._pack(mav, self.crc_extra, self.unpacker.pack(self.t_meas_us, self.ax, self.ay, self.az, self.gx, self.gy, self.gz, self.gap), force_mavlink1=force_mavlink1)


# Define name on the class for backwards compatibility (it is now msgname).
# Done with setattr to hide the class variable from mypy.
setattr(MAVLink_lawn_imu_raw_message, "name", mavlink_msg_deprecated_name_property())
...
mavlink_map: Dict[int, Type[MAVLink_message]] = {
    ...
    MAVLINK_MSG_ID_LAWN_FAULT_EVENT: MAVLink_lawn_fault_event_message,
    MAVLINK_MSG_ID_LAWN_TIMESYNC: MAVLink_lawn_timesync_message,
    MAVLINK_MSG_ID_LAWN_IMU_RAW: MAVLink_lawn_imu_raw_message,
...
class MAVLink(object):
    ...
    def lawn_drive_cmd_encode(self, left_drpm: int, right_drpm: int) -> MAVLink_lawn_drive_cmd_message:
        """
        Wheel speed request, high-level to low-level. A request, not a
        command:       the low-level tier decides whether to honour it
        (ADR-0001). Rejected unless fresh       (SAF-52) and the
        machine is armed (SAF-11).

        left_drpm                 : Left wheel target, deci-rpm. Signed. [drpm] (type:int16_t)
        right_drpm                : Right wheel target, deci-rpm. Signed. [drpm] (type:int16_t)

        """
        return MAVLink_lawn_drive_cmd_message(left_drpm, right_drpm)

    def lawn_drive_cmd_send(self, left_drpm: int, right_drpm: int, force_mavlink1: bool = False) -> None:
        """
        Wheel speed request, high-level to low-level. A request, not a
        command:       the low-level tier decides whether to honour it
        (ADR-0001). Rejected unless fresh       (SAF-52) and the
        ...
        """
        self.send(self.lawn_drive_cmd_encode(left_drpm, right_drpm), force_mavlink1=force_mavlink1)

    def lawn_arm_cmd_encode(self, magic: int, arm: int) -> MAVLink_lawn_arm_cmd_message:
        """
        Explicit arming. SAF-11 forbids driving on power-up without this.

        magic                     : Must be 0xA57E to arm. Any other value disarms.
      Guards against a corrupted or accidental frame enabling drive. (type:uint16_t)
        arm                       : 1 = arm, 0 = disarm. (type:uint8_t)

        """
        return MAVLink_lawn_arm_cmd_message(magic, arm)

    def lawn_arm_cmd_send(self, magic: int, arm: int, force_mavlink1: bool = False) -> None:
        """
        Explicit arming. SAF-11 forbids driving on power-up without this.
      ...
      Guards against a corrupted or accidental frame enabling drive. (type:uint16_t)
        ...
        """
        self.send(self.lawn_arm_cmd_encode(magic, arm), force_mavlink1=force_mavlink1)

    def lawn_stop_req_encode(self, reason: int) -> MAVLink_lawn_stop_req_message:
        """
        Request an immediate ramp to zero and disarm. NOT an emergency stop:
        SAF-40 requires a stop independent of both processors, which
        this is not. Named to       prevent it being mistaken for one.

        reason                    : Diagnostic only. (type:uint8_t)

        """
        return MAVLink_lawn_stop_req_message(reason)

    def lawn_stop_req_send(self, reason: int, force_mavlink1: bool = False) -> None:
        """
        Request an immediate ramp to zero and disarm. NOT an emergency stop:
        SAF-40 requires a stop independent of both processors, which
        this is not. Named to       prevent it being mistaken for one.
    ...
    def lawn_nav_status_encode(self, t_meas_us: int, cmd_age_ms: int, nav_state: int, armed: int, fault: int) -> MAVLink_lawn_nav_status_message:
        ...
        t_meas_us                 : Low-level monotonic clock. ADR-0007. [us] (type:uint64_t)
        cmd_age_ms                : Age of the last accepted drive
      request, on the low-level clock. 65535 = none ever accepted. [ms] (type:uint16_t)
        nav_state                 :  (type:uint8_t, values:LAWN_NAV_STATE)
        armed                     : 1 = drive enable asserted. (type:uint8_t)
        fault                     : Currently latched fault. (type:uint8_t, values:LAWN_FAULT_CODE)

        """
        return MAVLink_lawn_nav_status_message(t_meas_us, cmd_age_ms, nav_state, armed, fault)

    def lawn_nav_status_send(self, t_meas_us: int, cmd_age_ms: int, nav_state: int, armed: int, fault: int, force_mavlink1: bool = False) -> None:
        """
        Navigator state. Sent at the telemetry rate.
      ...
      request, on the low-level clock. 65535 = none ever accepted. [ms] (type:uint16_t)
        ...
        """
        self.send(self.lawn_nav_status_encode(t_meas_us, cmd_age_ms, nav_state, armed, fault), force_mavlink1=force_mavlink1)

    def lawn_wheel_state_encode(self, t_meas_us: int, left_drpm: int, right_drpm: int, left_cmd_drpm: int, right_cmd_drpm: int, flags: int) -> MAVLink_lawn_wheel_state_message:
        """
        Measured and commanded wheel speeds, timestamped at measurement.

        t_meas_us                 : Time the speed was measured, not sent. [us] (type:uint64_t)
        left_drpm                 : Measured left, deci-rpm. [drpm] (type:int16_t)
        right_drpm                : Measured right, deci-rpm. [drpm] (type:int16_t)
        left_cmd_drpm             : Applied left target after limiting. [drpm] (type:int16_t)
        right_cmd_drpm            : Applied right target after limiting. [drpm] (type:int16_t)
        flags                     :  (type:uint8_t, values:LAWN_WHEEL_FLAGS)

        """
        return MAVLink_lawn_wheel_state_message(t_meas_us, left_drpm, right_drpm, left_cmd_drpm, right_cmd_drpm, flags)

    def lawn_wheel_state_send(self, t_meas_us: int, left_drpm: int, right_drpm: int, left_cmd_drpm: int, right_cmd_drpm: int, flags: int, force_mavlink1: bool = False) -> None:
        """
        Measured and commanded wheel speeds, timestamped at measurement.
        ...
        """
        self.send(self.lawn_wheel_state_encode(t_meas_us, left_drpm, right_drpm, left_cmd_drpm, right_cmd_drpm, flags), force_mavlink1=force_mavlink1)

    def lawn_link_stats_encode(self, t_meas_us: int, frames_accepted: int, frames_rejected: int, dropped_tx: int, window_ms: int, heartbeats_missed: int, quality: int) -> MAVLink_lawn_link_stats_message:
        """
        Windowed link quality. SAF-54 and SAF-55: a degrading link must be
        observable before it becomes a stop.

        t_meas_us                 :  [us] (type:uint64_t)
        frames_accepted           : Accepted this window. (type:uint32_t)
        frames_rejected           : CRC, framing, or identity rejects this window. (type:uint32_t)
        dropped_tx                : Frames the low-level tier discarded because a
      transmit buffer was full. SAF-50 drop-on-full. (type:uint32_t)
        window_ms                 : Width of the measurement window. [ms] (type:uint16_t)
        heartbeats_missed         : Missed this window. (type:uint16_t)
        quality                   : 0-100. The value the degrade decision uses. (type:uint8_t)

        """
        return MAVLink_lawn_link_stats_message(t_meas_us, frames_accepted, frames_rejected, dropped_tx, window_ms, heartbeats_missed, quality)

    def lawn_link_stats_send(self, t_meas_us: int, frames_accepted: int, frames_rejected: int, dropped_tx: int, window_ms: int, heartbeats_missed: int, quality: int, force_mavlink1: bool = False) -> None:
        """
        Windowed link quality. SAF-54 and SAF-55: a degrading link must be
        observable before it becomes a stop.
      ...
      transmit buffer was full. SAF-50 drop-on-full. (type:uint32_t)
        ...
        """
        self.send(self.lawn_link_stats_encode(t_meas_us, frames_accepted, frames_rejected, dropped_tx, window_ms, heartbeats_missed, quality), force_mavlink1=force_mavlink1)

    def lawn_fault_event_encode(self, t_meas_us: int, code: int, nav_state: int, latched: int) -> MAVLink_lawn_fault_event_message:
        """
        Fault transition. Sent on change, not periodically.

        t_meas_us                 :  [us] (type:uint64_t)
        code                      :  (type:uint16_t, values:LAWN_FAULT_CODE)
        nav_state                 :  (type:uint8_t, values:LAWN_NAV_STATE)
        latched                   : 1 = requires explicit clear. (type:uint8_t)

        """
        return MAVLink_lawn_fault_event_message(t_meas_us, code, nav_state, latched)

    def lawn_fault_event_send(self, t_meas_us: int, code: int, nav_state: int, latched: int, force_mavlink1: bool = False) -> None:
        """
        Fault transition. Sent on change, not periodically.
        ...
        """
        self.send(self.lawn_fault_event_encode(t_meas_us, code, nav_state, latched), force_mavlink1=force_mavlink1)

    def lawn_timesync_encode(self, t1_us: int, t2_us: int, t3_us: int, exchange_seq: int) -> MAVLink_lawn_timesync_message:
        """
        Four-timestamp round trip. Replaces standard TIMESYNC (111), which
        carries       only two timestamps and forces a symmetric-delay
        assumption. ADR-0007 fits offset AND       skew, so all four
        are carried.        Request (Pi): exchange_seq set, t1_us = Pi
        transmit time, t2_us = t3_us = 0.       Response (Pico):
        exchange_seq echoed, t1_us echoed unchanged, t2_us = receive
        time,       t3_us = transmit time.        t2_us is captured at
        the start-bit edge of the request's first byte; t3_us is
        captured       as the response's last byte leaves the shift
        register. t3_us is a DELIBERATE exception       to ADR-0007
        item 1: for a sync frame, transmit time IS the measurement.

        t1_us                     : Requester transmit, requester clock. [us] (type:uint64_t)
        t2_us                     : Responder receive, responder clock. 0 in a request. [us] (type:uint64_t)
        t3_us                     : Responder transmit, responder clock. 0 in a request. [us] (type:uint64_t)
        exchange_seq              : Matches response to request. (type:uint8_t)

        """
        return MAVLink_lawn_timesync_message(t1_us, t2_us, t3_us, exchange_seq)

    def lawn_timesync_send(self, t1_us: int, t2_us: int, t3_us: int, exchange_seq: int, force_mavlink1: bool = False) -> None:
        """
        Four-timestamp round trip. Replaces standard TIMESYNC (111), which
        carries       only two timestamps and forces a symmetric-delay
        assumption. ADR-0007 fits offset AND       skew, so all four
        ...
        """
        self.send(self.lawn_timesync_encode(t1_us, t2_us, t3_us, exchange_seq), force_mavlink1=force_mavlink1)

    def lawn_imu_raw_encode(self, t_meas_us: int, ax: int, ay: int, az: int, gx: int, gy: int, gz: int, gap: int) -> MAVLink_lawn_imu_raw_message:
        """
        Raw inertial sample, timestamped at the sensor data-ready edge.
        Carried on       the BULK channel (USB CDC), not the command
        link. ADR-0006 keeps raw acquisition low and       estimation
        high.

        t_meas_us                 : Data-ready edge, hardware captured. [us] (type:uint64_t)
        ax                        : Accelerometer X, raw counts. (type:int16_t)
        ay                        : Accelerometer Y, raw counts. (type:int16_t)
        az                        : Accelerometer Z, raw counts. (type:int16_t)
        gx                        : Gyroscope X, raw counts. (type:int16_t)
        gy                        : Gyroscope Y, raw counts. (type:int16_t)
        gz                        : Gyroscope Z, raw counts. (type:int16_t)
        gap                       : Samples lost immediately before this one, 255 = saturated.
      ADR-0007: gaps are reported, never silently interpolated. (type:uint8_t)

        """
        return MAVLink_lawn_imu_raw_message(t_meas_us, ax, ay, az, gx, gy, gz, gap)

    def lawn_imu_raw_send(self, t_meas_us: int, ax: int, ay: int, az: int, gx: int, gy: int, gz: int, gap: int, force_mavlink1: bool = False) -> None:
        """
        Raw inertial sample, timestamped at the sensor data-ready edge.
        Carried on       the BULK channel (USB CDC), not the command
        link. ADR-0006 keeps raw acquisition low and       estimation
      ...
      ADR-0007: gaps are reported, never silently interpolated. (type:uint8_t)
        ...
        """
        self.send(self.lawn_imu_raw_encode(t_meas_us, ax, ay, az, gx, gy, gz, gap), force_mavlink1=force_mavlink1)
