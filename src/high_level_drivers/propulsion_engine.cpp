...
void Propulsion::startup_test() {
    enum class SUBTASKS { INITIALIZATION, ENCODER_TEST, SUSPEND };
    ...
    for (;;) {
        ...
        case SUBTASKS::ENCODER_TEST: {
            ...
            if (left_rpm > 0 && right_rpm > 0) {
                Log::info("hardware_state", "Encoders working: %u %u", left_rpm,
                          right_rpm);
                hardware.set_left_pwm_output(0, false, false);
                hardware.set_right_pwm_output(0, false, false);
            } else {
                Log::info("hardware_state",
                          "Encoders failed startup test: %u %u", left_rpm,
                          right_rpm);
                hardware.set_left_pwm_output(0, false, false);
                hardware.set_right_pwm_output(0, false, false);
                test_step = SUBTASKS::SUSPEND;
