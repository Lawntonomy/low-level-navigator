...
void GpioDriver::gpio_start()
{
    ...
    init_pwm_output_pin(static_cast<uint>(config_defines::config_1::gpio_num::right_pwm_pin), config_defines::config_1::pwm_frequency);
    init_pwm_output_pin(static_cast<uint>(config_defines::config_1::gpio_num::left_pwm_pin), config_defines::config_1::pwm_frequency);
    ...
    irq_queue[static_cast<uint>(config_defines::config_1::gpio_num::left_encoder_pin)] = xQueueCreate(25, sizeof(uint16_t));
    irq_queue[static_cast<uint>(config_defines::config_1::gpio_num::right_encoder_pin)] = xQueueCreate(25, sizeof(uint16_t));
    xTaskCreate(GpioDriver::led_heartbeat_task, "led_heartbeat_task", 1000, this, tskIDLE_PRIORITY + 2UL, NULL);
    xTaskCreate(GpioDriver::neopixel_status_task, "neopixel_status_task", 1000, this, tskIDLE_PRIORITY + 2UL, NULL);
    xTaskCreate(GpioDriver::update_gpio_task, "update_gpio_task", 1000, this, tskIDLE_PRIORITY + 2UL, NULL);
}
...
/// @brief Watches gpio_update queue, if pin isn't updated within 300ms, io locks out
        ...
        if(xQueueReceive(hardware::gpio_update_queue, &update, 10))
        {
            ...
            if(update.timestamp + 300 < (time_us_32()/1000))
            {
                if(update.max_value >= update.value)
                {
                    ...
                    {
                        if(update.value != driver->gpio_updates[static_cast<uint>(update.pin)].value)
                        {
...
void GpioDriver::init_pwm_output_pin(uint16_t gpio, uint16_t frequency)
{
    ...
    gpio_updates[gpio] = hardware::gpio_update(static_cast<config_defines::config_1::gpio_num>(gpio), 0, 65535, 0);
...
void GpioDriver::init_gpio_output_pin(uint16_t gpio)
{
    ...
    gpio_updates[gpio] = hardware::gpio_update(static_cast<config_defines::config_1::gpio_num>(gpio), 0, 1, 0);
